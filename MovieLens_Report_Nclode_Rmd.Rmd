...
This is done by creating 6 individual models and generating an ensemble model which the RMSE is used to compare to the target.
...
Now that the additional columns added, split the edx set into the 2 further subsets _train_set_ and _test_set_
...
The equation for the rating of movie i by user u is:
  ...
  estimate that minimizes the RMSE is the least squares estimate of $\mu$ which is the average of all ratings:
...
## Model R6 - Add Year of Rating Bias

The charts for avg rating by the year the rating was added (timestamp) shows variation across
